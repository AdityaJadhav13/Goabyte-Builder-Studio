...
**D-1a — Brand assets: permission GRANTED (8 Aug 2026, amends D-1).** Aditya confirmed that Hacker House Goa branding and artwork are explicitly available for participant use. D-1's default no longer applies. We may use organiser artwork and marks; we still may not use another participant's work, and the product still never claims to BE official — it is an independent tool built for the trial, and the footer says so. _Evidence:_ to be filed in `docs/` — the permission record should live with the repo, because "an unofficial tool carrying official branding" is a question someone may reasonably ask later. _Consequence:_ the landing hero is now licensed HH Goa illustration; the footer claim that "no organiser logos, wordmarks or artwork are used" was corrected, since it had become false.

**D-1 — Brand assets: original / inspired-by.** _(Superseded by D-1a.)_ _(Resolves Q1.)_ hhgoa.com is used as a **visual reference only**. Visual access is not a licence. We do not reuse their fonts, logo, wordmark, illustrations, or artwork. Organizer-supplied assets may be used **only** where they were explicitly made available for participant use; absent that, we build an original HH-Goa-inspired system with open-licensed (OFL) fonts and original decorative assets. The product never claims or implies official status. _Alternative considered:_ reuse site assets scraped from the live site — rejected, no public brand-kit or licence page was found in the official material, and unlicensed reuse is a reputational risk that would outweigh any visual gain. _Future implication:_ if organizers later licence a brand kit, the design token layer and asset directory are the only things that change.
