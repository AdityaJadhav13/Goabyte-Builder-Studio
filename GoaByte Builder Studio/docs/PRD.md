...
**Status:** Draft for review · v0.1 · 7 August 2026
...
### P1 — Rhea, mobile-first participant *(primary — optimise for her)*
...
Notation: **→** user action · *italics* = system state.
...
→ Selects a photo. *Reading file* (skeleton, ~instant) → *Decoding* → *Ready.* Cropper appears with a good default crop already applied and the PFP format preselected. Total: under 1.5 s for a typical 3 MB JPG.

### J3 — HEIC upload (iPhone)
→ Selects a HEIC. *Decoding.* Native decode is attempted first; if the browser can handle it, the user never learns their file was unusual. If not: status changes to "Converting your iPhone photo…" with an `aria-live` announcement, `heic2any` is lazy-loaded, conversion runs, flow resumes at J2. If conversion fails: J5 with HEIC-specific recovery copy ("Your browser can't read HEIC files. Open the photo in Photos, share it as JPEG, and try again.").
...
### J10 — Share to X
→ Taps Share to X.
- **Mobile with Web Share Level 2:** native share sheet opens carrying the actual PNG *and* the caption; the caption is simultaneously copied to the clipboard, and the UI says so — because iOS frequently drops text when a file is attached.
- **Everywhere else:** the image downloads (if not already), `x.com/intent/post` opens pre-filled, and the UI states plainly: *"Your image is downloaded — attach it to the post."*
...
- **FR-010** Load `heic2any` **only** after a native decode of a HEIC/HEIF file has failed, via dynamic import.
...
- **FR-027** Keep the photo and name inside a horizontally centred 16:9 safe band, so the card survives aggressive timeline cropping.
...
- **NFR-035** Identical inputs produce a byte-identical export. No randomness, no timestamps, no `Date.now()` in the render path.
...
|---|---|---|---|
| Q1 | Do we have licensed HH Goa brand assets (logo, wordmark, fonts)? | Original "inspired by" system, OFL fonts, honest non-official attribution | **Design-blocking from Day 1** |
...
|---|---|---|---|
| D0 | Fri 7 Aug | Docs, repo, CI, technical spike on a real iPhone (HEIC, canvas limits, Web Share) | Empty app deployed to Vercel |
...
*Next document: `docs/ARCHITECTURE.md` — only after this PRD is reviewed.*
